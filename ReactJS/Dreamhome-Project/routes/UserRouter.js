...
router.get("/searchproduct",(req,res)=>{ 
  AdminController.fetchCategory({}).then((result)=>{
    //console.log(result);
    res.render("User/searchproduct",{"sunm":req.session.sunm,"clist":result});
  }).catch((err)=>{
    console.log(err);
  });
});
...
router.get("/viewsubcategory/:catname",(req,res)=>{ 
  ...
  UserController.fetchSubCategory(req.params).then((result)=>{
    //console.log(result);
    res.render("Admin/viewsubcategory",{"sunm":req.session.sunm,"clist":clist,"sclist":result,"catname":req.params.catname});
  }).catch((err)=>{
    console.log(err);
  });
});
...
router.get("/funds",(req,res)=>{ 
  ...
  res.render("User/funds",{"sunm":req.session.sunm,"paypalURL":paypalURL,"paypalID":paypalID,"amount":amount}); 
});
